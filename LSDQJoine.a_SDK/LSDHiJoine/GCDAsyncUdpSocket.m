...
- (void)notifyDidSendDataWithTag:(long)tag
{
	...
	if (delegateQueue && [delegate respondsToSelector:@selector(udpSocket:didSendDataWithTag:)])
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
//            NSLog(@"delegateQueue NSThread%@", [NSThread currentThread]);
...
- (BOOL)enableBroadcast:(BOOL)flag error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (socket4FD != SOCKET_NULL)
		{
//			int value = flag ? 1 : 0;//此处设置 ttl 的值 所以可以直接使用TTL
            int value = flag ? TTL : 0;
...
- (void)endCurrentSend
{
    ...
    currentSend -> buffer = nil;
    currentSend -> address = nil;
